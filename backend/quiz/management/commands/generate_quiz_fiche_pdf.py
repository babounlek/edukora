...
from quiz.pdf import save_quiz_fiche_pdf


class Command(BaseCommand):
    help = (
        "Génère (hors ligne) le PDF fiche (énoncés+corrigé) d'une QuizSession terminée - "
        "déclenchée par quiz.views.quiz_fiche_pdf via queue_quiz_fiche_pdf_generation. Ne "
        "tourne jamais dans le cycle d'une requête HTTP."
    )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f"Session #{session.pk} : PDF fiche généré."))
