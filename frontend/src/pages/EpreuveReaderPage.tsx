...
export function EpreuveReaderPage() {
  ...
  useEffect(() => {
    if (isLoading) return
    if (!isAuthenticated) {
      navigate("/connexion", { state: { from: `/epreuves/${slug}/lire` } })
      return
    }
    if (!slug) return
  ...
  useEffect(() => {
    // Requête publique séparée de readEpreuve (réservée aux abonnés) : sert uniquement
    // à alimenter "Autres épreuves" (subject.code/cursus, absents de EpreuveContent.header
    // qui ne porte que des libellés d'affichage) - voir RelatedEpreuves.
    if (!slug) return
    getEpreuve(slug).then(setEpreuve).catch(() => {})
  }, [slug])
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
      <div className="mb-6 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-muted-foreground">
        ...
        <Link to={`/epreuves/${slug}`} className="transition-colors hover:text-primary">
      ...
      <BackToTopBar
        links={[
          { to: catalogueHomePath(country), label: "Retour au catalogue" },
          { to: `/epreuves/${slug}`, label: "Retour à la fiche de l'épreuve" },
        ]}
      />
    </div>
  )
